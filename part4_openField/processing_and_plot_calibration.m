...
% 
load("Dados\18_fevereiro\calibracao\calibracao_final.mat");
load("mic4189.mat");
addpath("codigos_auxiliares\");

%% 

% audiowrite("Dados\18_fevereiro\calibracao\calibracao_final.wav", Recording(:, 1), Fs);

%% Processamento

weighting = 'flat'; % 'A'; 'C', 'flat
bandtype = 'third'; % oct, third
filename = 'Dados\18_fevereiro\calibracao\calibracao_final.wav';
...
plot(t, Recording)
title('Sinal de tensão do microfone (calibração - final)');
xlabel('Tempo [s]'); ylabel('Tensão [V]');
text = sprintf('Sensibilidade %.4f [V/Pa]', sens);
legend(text, 'Location', 'northeast')
% print(gcf, '-dpng', '-r600', 'Figuras\calibracao_final_tensão.png')
...
title(sprintf('Calibração no final das medições'))
% print(gcf, '-dpng', '-r600', 'Figuras\calibracao_final_frequencia.png')

%% Plot mic correction facton (with and without interpolation)

figure('Name', 'Correção mic (sem interp)')
semilogx(mic4189(:, 1), mic4189(:, 2)); grid on;
...
% print(gcf, '-dpng', '-r600', 'Figuras\correcao_mic4189_FreeField.png')

figure('Name', 'Correção mic (com e sem interp)')
semilogx(mic4189(:, 1), mic4189(:, 2)); hold on; grid on;
...
% print(gcf, '-dpng', '-r600', 'Figuras\correcao_mic4189_FreeField_comparativo.png')