function varargout=shadedErrorBar_muCI(x,y,errBar,lineProps,transparent,patchSaturation,xaxis,edges)
% New implementations by Dr. William D'Andrea Fonseca
% It is possible to select the patchSaturation, xaxis and edges
% xaxis options: lin, log, lin-no AND log-no
% The "no" options wont plot the mean, for example.
%
% Atualização: 11/08/2017
%
...
if nargin<5, transparent=0; 
   patchSaturation=0.10; xaxis='lin';  end

if nargin<6, patchSaturation=0.10; xaxis='lin';  end

if nargin<7, xaxis='lin'; end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
% Plot to get the parameters of the line 
if strcmp(xaxis,'lin')
    H.mainLine=plot(x,y,lineProps{:});
elseif strcmp(xaxis,'log')
    H.mainLine=semilogx(x,y,lineProps{:});
elseif strcmp(xaxis,'log-no')
    H.mainLine=semilogx(x,y,lineProps{:});
else 
    H.mainLine=plot(x,y,lineProps{:});
end
...
col=get(H.mainLine,'color'); if iscell(col); col = col{1}; end;
edgeColor=col(1)+(1-col(1))*0.55;
% patchSaturation=0.120; %How de-saturated or transparent to make patch
if transparent
...
if strcmp(xaxis,'lin')
    H.mainLine=plot(x,y,lineProps{:});
elseif strcmp(xaxis,'lin-no')
    % Will not plot main line
elseif strcmp(xaxis,'log')
    H.mainLine=semilogx(x,y,lineProps{:});
elseif strcmp(xaxis,'log-no')    
    % Will not plot main line
