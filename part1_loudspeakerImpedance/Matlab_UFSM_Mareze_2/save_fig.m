function save_fig(Nome,mode,Fig,SavePdf,SavePng,SaveJpg,SaveFig,resolution,xaxis,fechar,append,extraSpace)
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%
%   Exemplo:
%   save_fig(Nome,[1 1],Plot.fig,1,1,0,0,200,Plot.Tipo,0,0);
...
elseif ~isempty(strfind(Nome,':')); Nome = strrep(Nome,':','-'); end
if nargin<2; mode=1; Fig = gcf; SavePdf=1; SavePng=0; SaveFig=0; SaveJpg=0; resolution=200; xaxis=''; fechar=0; append=0; end;
...
if SavePng == 1
   set(gcf, 'color', 'none');
   export_fig(Fig,sprintf('%s', Nome), '-png', ['-r' int2str(resolution)], '-q99','-transparent');
end
...
if mode==2 
    disp('I using the Matlab ''print'' function to save the figure(s).');
if SavePdf == 1 
% ax = Fig.CurrentAxes; fig = gcf;
