...
Confianca1 = 99;
[abs_CI, abs_conf] = stat_conf(Hf','T',Confianca1,0,1,'nodB','conj'); % t-student
abs_inf=eval('abs_CI.muCI(:,1)'); abs_sup=eval('abs_CI.muCI(:,2)');

f.spanLog = [freq]; f.plot=[50, 1000:1000:5000];
figure(10)
Plot.xlabel = 'Frequência [Hz]'; Plot.ylabel = 'Coef. de Absorção [-]';
Plot.shade1 = shadedErrorBar(freq, Hf', {@mean, @(freq) abs(abs_CI.interval)}, {'Color',[0/255 30/255 255/255],'LineWidth', 3.0}, 1, 0.25, 'log', 0); hold on;      
f.spanPoints=round(logspace(log10(50),log10(20000),50));
