...
%%% Vetores 
data.Amostra.vol = Qms;   %% dados resistividade volumétrica 
...
PlotD.Nome = ['Qms ' ' - ' tipo_med ' - ' num_med]; PlotD.fig = figure('Name',PlotD.Nome);    
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
  xlabel ('Medições','FontSize',10); ylabel ('Qms [-]'); 
   title({['Qms']; ['Amostra ' ' - ' tipo_med ' - ' num_med]}); PlotD.fig.CurrentAxes.XTick = 0:1:length(data.Amostra.vol)+1; xlim([0 length(data.Amostra.vol)+1]); arruma_fig('%1.0f','%2.3f');
    ax = gca;
...
ylim([3.8 4.1])
yticks([3.8, 3.9, 4.0, 4.1]); yticklabels({'3,800', '3,900', '4,000', '4,100'})
print(gcf, '-dpng', '-r300', '..\..\Figuras\Parametros\Qms_variacao.png')
