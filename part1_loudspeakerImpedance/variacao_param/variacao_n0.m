...
%%% Vetores 
data.Amostra.vol = n0;   %% dados resistividade volumétrica 
...
PlotD.Nome = ['n0 ' ' - ' tipo_med ' - ' num_med]; PlotD.fig = figure('Name',PlotD.Nome);    
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
  xlabel ('Medições','FontSize',10); ylabel ('\eta_{0} [%]'); 
   title({['\eta_{0}']; ['Amostra ' ' - ' tipo_med ' - ' num_med]}); PlotD.fig.CurrentAxes.XTick = 0:1:length(data.Amostra.vol)+1; xlim([0 length(data.Amostra.vol)+1]); arruma_fig('%1.0f','%2.3f');
    ax = gca;
...
ylim([.22 .26])
yticks([.22, .23, .24, .25, .26]); yticklabels({'0,220', '0,230', '0,240', '0,250', '0,260'})
print(gcf, '-dpng', '-r300', '..\..\Figuras\Parametros\n0_variacao.png')
