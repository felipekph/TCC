...
%%% Vetores 
data.Amostra.vol = Qts;   %% dados resistividade volumétrica 
...
PlotD.Nome = ['Qts ' ' - ' tipo_med ' - ' num_med]; PlotD.fig = figure('Name',PlotD.Nome);    
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
  xlabel ('Medições','FontSize',10); ylabel ('Qts [-]'); 
   title({['Qts']; ['Amostra ' ' - ' tipo_med ' - ' num_med]}); PlotD.fig.CurrentAxes.XTick = 0:1:length(data.Amostra.vol)+1; xlim([0 length(data.Amostra.vol)+1]); arruma_fig('%1.0f','%2.3f');
    ax = gca;
...
%%% Legenda %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
warning off 
lgn = legend(['M\''edia, $\bar{X}$: ' strrep(num2str(mean(data.Amostra.vol),'%1.3f'), '.', ',') ' [-]'], ...
      [num2str(Res.Amostra.volCImed1.confidence) '$\%$-Med., $\bar{X}\,-$'  strrep(num2str(Res.Amostra.volCImed1.z(1,2),'%3.2f'), '.', ',') '$*S\bar{X}$: ' strrep(num2str(Res.Amostra.volCImed1.muCI(1),'%1.3f'), '.', ',') ' [-]'], ... 
      [num2str(Res.Amostra.volCImed1.confidence) '$\%$-Med., $\bar{X}\,+$'  strrep(num2str(Res.Amostra.volCImed1.z(1,2),'%3.2f'), '.', ',') '$*S\bar{X}$: ' strrep(num2str(Res.Amostra.volCImed1.muCI(2),'%1.3f'), '.', ',') ' [-]'], ...
      [num2str(Res.Amostra.volCIconj1.confidence) '$\%$-Conj, $\bar{X}\,-$' strrep(num2str(Res.Amostra.volCIconj1.z(1,2),'%3.2f'), '.', ',') '$*S{X}$: '    strrep(num2str(Res.Amostra.volCIconj1.muCI(1),'%1.3f'), '.', ',') ' [-]'], ...
      [num2str(Res.Amostra.volCIconj1.confidence) '$\%$-Conj, $\bar{X}\,+$' strrep(num2str(Res.Amostra.volCIconj1.z(1,2),'%3.2f'), '.', ',') '$*S{X}$: '    strrep(num2str(Res.Amostra.volCIconj1.muCI(2),'%1.3f'), '.', ',') ' [-]']);    
...
ylim([.92 1])
yticks([.92, .94, .96, .98, 1]); yticklabels({'0,920', '0,940', '0,960', '0,980', '1,000'})
print(gcf, '-dpng', '-r300', '..\..\Figuras\Parametros\Qts_variacao.png')
