...
%%% Vetores 
data.Amostra.vol = L_1kHz;   %% dados resistividade volumétrica 
...
PlotD.Nome = ['L_1kHz ' ' - ' tipo_med ' - ' num_med]; PlotD.fig = figure('Name',PlotD.Nome);    
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
  xlabel ('Medições','FontSize',10); ylabel ('L @ 1kHz [mH]'); 
   title({['L @ 1kHz']; ['Amostra ' ' - ' tipo_med ' - ' num_med]}); PlotD.fig.CurrentAxes.XTick = 0:1:length(data.Amostra.vol)+1; xlim([0 length(data.Amostra.vol)+1]); arruma_fig('%1.0f','%2.3f');
    ax = gca;
...
ylim([.45 .52])
yticks([.45, .5, .52]); yticklabels({'0,450', '0,500', '0,520'})
print(gcf, '-dpng', '-r300', '..\..\Figuras\Parametros\L_1kHz_variacao.png')
