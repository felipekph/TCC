...
%%% Vetores 
data.Amostra.vol = L_20kHz;   %% dados resistividade volumétrica 
...
PlotD.Nome = ['L @ 20kHz ' ' - ' tipo_med ' - ' num_med]; PlotD.fig = figure('Name',PlotD.Nome);    
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
  xlabel ('Medições','FontSize',10); ylabel ('L @ 20kHz [mH]'); 
   title({['L @ 20kHz']; ['Amostra ' ' - ' tipo_med ' - ' num_med]}); PlotD.fig.CurrentAxes.XTick = 0:1:length(data.Amostra.vol)+1; xlim([0 length(data.Amostra.vol)+1]); arruma_fig('%1.0f','%2.3f');
    ax = gca;
...
ylim([.18 .2])
yticks([.18, .19, .2]); yticklabels({'0,180', '0,190', '0,200'})
print(gcf, '-dpng', '-r300', '..\..\Figuras\Parametros\L_20kHz_variacao.png')
