...
%%% Vetores 
data.Amostra.vol = Qes;   %% dados resistividade volumétrica 
...
PlotD.Nome = ['Qes ' ' - ' tipo_med ' - ' num_med]; PlotD.fig = figure('Name',PlotD.Nome);    
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
  xlabel ('Medições','FontSize',10); ylabel ('Qes [-]'); 
   title({['Qes']; ['Amostra ' ' - ' tipo_med ' - ' num_med]}); PlotD.fig.CurrentAxes.XTick = 0:1:length(data.Amostra.vol)+1; xlim([0 length(data.Amostra.vol)+1]); arruma_fig('%1.0f','%2.3f');
    ax = gca;
...
ylim([1.1 1.35])
yticks([1.1, 1.2, 1.3, 1.35]); yticklabels({'1,100', '1,200', '1,300', '1,350'})
print(gcf, '-dpng', '-r300', '..\..\Figuras\Parametros\Qes_variacao.png')
