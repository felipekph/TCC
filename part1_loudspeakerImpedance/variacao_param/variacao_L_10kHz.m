...
%%% Vetores 
data.Amostra.vol = L_10kHz;   %% dados resistividade volumétrica 
...
PlotD.Nome = ['L @ 10kHz ' ' - ' tipo_med ' - ' num_med]; PlotD.fig = figure('Name',PlotD.Nome);    
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
  xlabel ('Medições','FontSize',10); ylabel ('L @ 10kHz [mH]'); 
   title({['L @ 10kHz']; ['Amostra ' ' - ' tipo_med ' - ' num_med]}); PlotD.fig.CurrentAxes.XTick = 0:1:length(data.Amostra.vol)+1; xlim([0 length(data.Amostra.vol)+1]); arruma_fig('%1.0f','%2.3f');
    ax = gca;
...
%%% Legenda %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
warning off 
lgn = legend(['M\''edia, $\bar{X}$: ' strrep(num2str(mean(data.Amostra.vol),'%1.3f'), '.', ',') ' [mH]'], ...
      [num2str(Res.Amostra.volCImed1.confidence) '$\%$-Med., $\bar{X}\,-$'  strrep(num2str(Res.Amostra.volCImed1.z(1,2),'%3.2f'), '.', ',') '$*S\bar{X}$: ' strrep(num2str(Res.Amostra.volCImed1.muCI(1),'%1.3f'), '.', ',') ' [mH]'], ... 
      [num2str(Res.Amostra.volCImed1.confidence) '$\%$-Med., $\bar{X}\,+$'  strrep(num2str(Res.Amostra.volCImed1.z(1,2),'%3.2f'), '.', ',') '$*S\bar{X}$: ' strrep(num2str(Res.Amostra.volCImed1.muCI(2),'%1.3f'), '.', ',') ' [mH]'], ...
      [num2str(Res.Amostra.volCIconj1.confidence) '$\%$-Conj, $\bar{X}\,-$' strrep(num2str(Res.Amostra.volCIconj1.z(1,2),'%3.2f'), '.', ',') '$*S{X}$: '    strrep(num2str(Res.Amostra.volCIconj1.muCI(1),'%1.3f'), '.', ',') ' [mH]'], ...
      [num2str(Res.Amostra.volCIconj1.confidence) '$\%$-Conj, $\bar{X}\,+$' strrep(num2str(Res.Amostra.volCIconj1.z(1,2),'%3.2f'), '.', ',') '$*S{X}$: '    strrep(num2str(Res.Amostra.volCIconj1.muCI(2),'%1.3f'), '.', ',') ' [mH]']);    
...
%print(gcf, '-dpng', '-r600', 'Figuras\Qms_variacao.png')
%% Dados Amb
...
ylim([.2 .255])
yticks([.2, .25, .255]); yticklabels({'0,200', '0,250', '0,255'})
print(gcf, '-dpng', '-r300', '..\..\Figuras\Parametros\L_10kHz_variacao.png')
