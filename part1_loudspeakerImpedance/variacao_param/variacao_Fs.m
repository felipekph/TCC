...
%%% Vetores 
data.Amostra.vol = Fs;   %% dados resistividade volumétrica 
...
PlotD.Nome = ['Fs ' ' - ' tipo_med ' - ' num_med]; PlotD.fig = figure('Name',PlotD.Nome);    
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
  xlabel ('Medições','FontSize',10); ylabel ('Fs [Hz]'); 
   title({['Fs']; ['Amostra ' ' - ' tipo_med ' - ' num_med]}); PlotD.fig.CurrentAxes.XTick = 0:1:length(data.Amostra.vol)+1; xlim([0 length(data.Amostra.vol)+1]); arruma_fig('%1.0f','%2.3f');
    ax = gca;
...
%%% Legenda %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
warning off 
lgn = legend(['M\''edia, $\bar{X}$: ' strrep(num2str(mean(data.Amostra.vol),'%1.3f'), '.', ',') ' [Hz]'], ...
      [num2str(Res.Amostra.volCImed1.confidence) '$\%$-Med., $\bar{X}\,-$'  strrep(num2str(Res.Amostra.volCImed1.z(1,2),'%3.2f'), '.', ',') '$*S\bar{X}$: ' strrep(num2str(Res.Amostra.volCImed1.muCI(1),'%1.3f'), '.', ',') ' [Hz]'], ... 
      [num2str(Res.Amostra.volCImed1.confidence) '$\%$-Med., $\bar{X}\,+$'  strrep(num2str(Res.Amostra.volCImed1.z(1,2),'%3.2f'), '.', ',') '$*S\bar{X}$: ' strrep(num2str(Res.Amostra.volCImed1.muCI(2),'%1.3f'), '.', ',') ' [Hz]'], ...
      [num2str(Res.Amostra.volCIconj1.confidence) '$\%$-Conj, $\bar{X}\,-$' strrep(num2str(Res.Amostra.volCIconj1.z(1,2),'%3.2f'), '.', ',') '$*S{X}$: '    strrep(num2str(Res.Amostra.volCIconj1.muCI(1),'%1.3f'), '.', ',') ' [Hz]'], ...
      [num2str(Res.Amostra.volCIconj1.confidence) '$\%$-Conj, $\bar{X}\,+$' strrep(num2str(Res.Amostra.volCIconj1.z(1,2),'%3.2f'), '.', ',') '$*S{X}$: '    strrep(num2str(Res.Amostra.volCIconj1.muCI(2),'%1.3f'), '.', ',') ' [Hz]']);    
...
ylim([60 67])
yticks([60, 61, 62, 63, 64, 65, 66, 67]); yticklabels({'60,00', '61,00', '62,00', '63,00', '64,00', ...
    '65,00', '66,00', '67,00'})
print(gcf, '-dpng', '-r300', '..\..\Figuras\Parametros\Fs_variacao.png')
